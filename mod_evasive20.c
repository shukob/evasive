...
static int log_only = DEFAULT_LOG_ONLY;

TRex **targeted_url_regex_list = NULL; //Regex list for target url
static int targeted_url_regex_count = 0;
TRex **untargeted_url_regex_list = NULL; //Regex list for untargeted url
static int untargeted_url_regex_count = 0;
...
static const char *targetedURL(cmd_parms *cmd, void *dconfig, const char *regex){
  const TRexChar *error = NULL;
  TRex * trex = trex_compile(_TREXC(regex), &error);
  if(trex){
    targeted_url_regex_list = (TRex**)realloc(targeted_url_regex_list, (targeted_url_regex_count + 1) * sizeof(TRex*));
    targeted_url_regex_list[targeted_url_regex_count]=trex;
    targeted_url_regex_count += 1;
  }
  return NULL;
}

static const char *untargetedURL(cmd_parms *cmd, void *dconfig, const char *regex){
  const TRexChar *error = NULL;
  TRex * trex = trex_compile(_TREXC(regex), &error);
  if(trex){
    untargeted_url_regex_list = (TRex**)realloc(untargeted_url_regex_list, (untargeted_url_regex_count + 1) * sizeof(TRex*));
    untargeted_url_regex_list[untargeted_url_regex_count]=trex;
    untargeted_url_regex_count += 1;
  }
  return NULL;
}
...
/* make example.com:80/some/path like expression of destination */
static void makeURLExpression(request_rec* r, char* buf, int buf_length){
  const char* hostname = r->hostname;
  const int port = r->server->port;
  const char* uri = r->uri;
  snprintf(buf, buf_length, "%s:%d%s", hostname, port, uri);
}

static int destination_is_filtered(request_rec *r){
    char urlExpression[4096];
    makeURLExpression(r, urlExpression, 4096);
    int filtered = 0;
    /* Check if this url is untargeted and filter if such*/
    if(untargeted_url_regex_count > 0){
      int i = 0;
      for(i = 0;i < untargeted_url_regex_count; ++i){
        TRex *trex = untargeted_url_regex_list[i];
        if(trex_match(trex, _TREXC(urlExpression))){
          filtered = 1;
          break;
        }
      }
    ...
    /* Check if this url is targeted and unfilter if such*/
    if(!filtered){
      if(targeted_url_regex_count > 0){
        int targeted = 0;
        int i = 0;
        for(i = 0; i < targeted_url_regex_count ; ++i){
          TRex *trex = targeted_url_regex_list[i];
          if(trex_match(trex, _TREXC(urlExpression))){
            targeted = 1;
            break;
          }
...
static int access_checker(request_rec *r) 
{
    ...
    char extract_fowarded_for_buffer[128];

    if(x_forwarded_for_as_remote_ip && (forwarded_for = apr_table_get(r->headers_in, "X-Forwarded-For")) != NULL){
        if(extract_client_ip_from_x_forwarded_for(forwarded_for, extract_fowarded_for_buffer, 128)){
...
static apr_status_t cleanup_regex_list(void *not_used){
  int i = 0;
  for(i = 0 ; i < targeted_url_regex_count ; ++i){
    trex_free(targeted_url_regex_list[i]);
  }
  targeted_url_regex_count = 0;
  ...
  for(i = 0 ; i < untargeted_url_regex_count ; ++i){
    trex_free(untargeted_url_regex_list[i]);
  }
  untargeted_url_regex_count = 0;

  if(untargeted_url_regex_list){
    free(untargeted_url_regex_list);
    untargeted_url_regex_list = NULL;
  }
}
...
static const char*
get_log_only(cmd_parms *cmd, void *dconfig, const char *value){
  long n = strtol(value, NULL, 0);
  if (n>0) {
    log_only = 1;
  } else {
...
static const command_rec access_cmds[] =
{
  ...
  AP_INIT_ITERATE("DOSTargetedURL", targetedURL, NULL, RSRC_CONF,
    "URL regex to target"),

  AP_INIT_ITERATE("DOSUnTargetedURL", untargetedURL, NULL, RSRC_CONF,
    "URL regex to untarget"),

  AP_INIT_TAKE1("DOSLogOnly", get_log_only, NULL, RSRC_CONF,
    "Set no fobidding for detected Dos"),
